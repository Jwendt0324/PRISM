# Where Humans Are Required — BlitzMetrics Canon

**Source:** All BlitzMetrics guides, Dennis Yu's feedback, operational reality
**Version:** 1.0 | **Extracted:** 2026-03-19

---

## Purpose

Dennis's directive: "Consider whether this can operate without humans in the loop." This document explicitly defines where humans are REQUIRED in every process. AI and automation can assist but cannot replace human judgment in these areas.

---

## Content Production (Stage 1: Produce)

| Task | Why Human Required |
|------|--------------------|
| Filming video content | Only the business owner can tell their stories |
| Photography at events/work | Must be physically present, geotagged |
| Client interviews | Relationship and rapport required |
| WHY video recording | Personal mission can't be delegated |
| Podcast hosting/guesting | Real-time conversation skills |
| Speaking at events | Physical presence, audience engagement |

**Rule:** The business owner must produce the raw content. This is the 1% they must do. Everything else can be delegated.

---

## Content Quality (Stage 2: Process)

| Task | Why Human Required |
|------|--------------------|
| Watching source video before writing | Context, tone, intent not captured in transcript alone |
| Name/brand spelling verification | AI can't verify against real-world spelling |
| Voice and tone approval | "Would Jack actually say this?" |
| E-E-A-T compliance review | Ensuring real stories, not fabricated ones |
| AI language detection | Subtle AI patterns require trained eye |
| Figurehead context | Understanding relationships between people mentioned |

**Rule:** Dennis flagged AI language in Jackson's initial work. Every AI-assisted piece must be reviewed by a human who knows the client's voice.

---

## Publishing (Stage 3: Post)

| Task | Why Human Required |
|------|--------------------|
| WordPress login and publishing | Credentials, featured image upload, RankMath settings |
| Category/tag selection | Must align with client's Topic Wheel strategy |
| Featured image selection | Must be real, relevant photo (not stock) |
| Schema.org validation | Technical verification |
| Content tree verification | Ensuring no keyword cannibalization (anti-vandalism) |

**Rule:** No article should auto-publish without human review and approval.

---

## Client Relationships

| Task | Why Human Required |
|------|--------------------|
| Client onboarding (GCT form) | Client must articulate goals themselves |
| Discovery/sales calls | Trust built through personal interaction |
| Pricing discussions | Judgment, negotiation, no discounting |
| Client escalation handling | Empathy, context, relationship preservation |
| Monthly review calls | Maintaining relationship, reading client satisfaction |
| Qualification review (AI Apprentice) | Checking for AI language, assessing effort |

**Rule:** Dennis's directive: "Clients first." Client relationships cannot be automated.

---

## Financial Operations

| Task | Why Human Required |
|------|--------------------|
| Budget approval for ad scaling | Financial risk assessment |
| P&L tracking and reporting | Accuracy, compliance, Dennis pushing for this |
| Revenue distribution (BM/HRI split) | Legal agreement, judgment calls |
| Refund decisions | Case-by-case evaluation |
| Pricing strategy | Market judgment, positioning |
| Invoice review | Verification against deliverables |

**Rule:** No financial transaction should happen without human authorization.

---

## Strategic Decisions

| Task | Why Human Required |
|------|--------------------|
| Methodology changes | Dennis's review required |
| New service/product launches | Business judgment |
| Team hiring/firing | Relationship, culture fit |
| Partnership agreements | Legal, strategic alignment |
| Content strategy pivots | Market awareness, client needs |
| SOP changes that affect canon | Must verify against BlitzMetrics standards |

**Rule:** "Changes to methodology require Dennis's review."

---

## Quality Assurance

| Task | Why Human Required |
|------|--------------------|
| Final article review before publish | Catching what automated checks miss |
| MAA Analysis section | Insight requires human judgment |
| Authority scoring (3 Components) | Subjective assessment of What/Who/Where |
| Dollar-a-Day campaign decisions | Kill/keep/scale requires judgment |
| Client satisfaction assessment | Reading between the lines |

**Rule:** Automated QA catches mechanical issues. Human QA catches strategic issues.

---

## Summary Matrix

| Process | Fully Automatable | Needs Human QA | Fully Human |
|---------|------------------|----------------|-------------|
| Transcription | ✅ | | |
| Article writing (draft) | ✅ | | |
| Banned word checking | ✅ | | |
| Structural QA | ✅ | | |
| Internal link insertion | ✅ | | |
| Content Library logging | ✅ | | |
| Article voice/tone review | | ✅ | |
| E-E-A-T compliance | | ✅ | |
| SEO strategy | | ✅ | |
| WordPress publishing | | | ✅ |
| Featured image selection | | | ✅ |
| Client communication | | | ✅ |
| Filming/photography | | | ✅ |
| Financial decisions | | | ✅ |
| MAA Analysis | | | ✅ |
| Discovery calls | | | ✅ |
