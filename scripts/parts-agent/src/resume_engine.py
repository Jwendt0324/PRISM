...
Tracks progress during a run so that if the agent crashes, it can
resume from where it left off instead of re-processing everything.
"""
...
class ResumeEngine:
    ...
    def check_for_incomplete_run(self, auto_resume=False):
        ...
        Returns:
            dict with progress data if resuming, None if starting fresh.
        """
        ...
        if completed >= total:
            # Previous run actually finished — clean up
    ...
    def start_new_run(self, part_numbers, input_files):
        """Create a new progress file for a fresh run.

        Args:
            part_numbers: list of part number strings to process.
            input_files: list of input file paths (for reference).
        """
    ...
    def get_completed_parts(self):
        """Return set of part numbers already completed in this run."""
        if not self._progress:
            return set()
        return set(self._progress.get("completed", []))

    def get_remaining_parts(self):
        """Return list of part numbers not yet completed."""
        if not self._progress:
            return []
    ...
    def mark_completed(self, part_number):
        """Record that a part number was successfully processed."""
    ...
    def mark_failed(self, part_number):
        """Record that a part number failed after all retries."""
        if self._progress is None:
            return
        if part_number not in self._progress["failed"]:
            self._progress["failed"].append(part_number)
        # Also mark as completed so we don't retry on resume
        if part_number not in self._progress["completed"]:
            self._progress["completed"].append(part_number)
        self._progress["last_updated"] = datetime.now().isoformat()
        self._save()
    ...
    def finish(self):
        """Delete the progress file when the run completes successfully."""
    ...
    def _save(self):
        """Write the current progress to disk."""
