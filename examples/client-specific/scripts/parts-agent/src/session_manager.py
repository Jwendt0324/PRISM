...
class SessionManager:
    ...
    def __init__(self, config, distributor, log_fn, project_root):
        """
        Args:
            config: dict from config/config.json (credentials, settings)
            distributor: dict from config/distributors/<name>.json (selectors, URLs)
    ...
    def get_authenticated_page(self):
        ...
        Tries to reuse a saved session first. If that fails or no session
        exists, performs a fresh login and saves the new session.
        """
    ...
    def _try_restore_session(self):
        ...
        Returns True if the session is valid and the dashboard is ready.
        """
        try:
            ...
            # Check if we're still authenticated by looking for the dashboard ready selector
