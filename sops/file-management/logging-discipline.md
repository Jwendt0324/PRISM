---
description: Define when session logs are required, what quality standard they
  must meet, and how to prevent stub/empty logs from accumulating
...
- **PostToolUse hook** → `~/Documents/Claude/PRISM/logs/actions/YYYY-MM-DD.jsonl` — one JSONL line per tool call with timestamp, tool name, target (file path or command), and session ID. ~150 bytes per event.
- **SessionStart hook** → `~/Documents/Claude/PRISM/logs/session-events.jsonl` — session start events
- **SessionEnd hook** → calls `parse-session.py` v3, which produces:
  - `~/Documents/Claude/PRISM/logs/sessions/YYYY-MM/session-{ID}.jsonl` — machine-readable session data + full action sequence
  - `~/Documents/Claude/PRISM/logs/sessions/YYYY-MM/session-{ID}.md` — human-readable summary with all auto-captured fields
...
- **EOD:** `python3 ~/Documents/Claude/PRISM/scripts/generate-eod-report.py [date]` → `logs/reports/eod-YYYY-MM-DD.md`
- **Weekly:** `python3 ~/Documents/Claude/PRISM/scripts/generate-weekly-report.py [monday-date]` → `logs/reports/weekly-YYYY-WNN.md`
- **Log rotation:** `bash ~/Documents/Claude/PRISM/scripts/rotate-logs.sh` — compresses action logs >7 days, deletes >90 days
...
2. **Before session ends:** Find the auto-generated log at `~/Documents/Claude/PRISM/logs/sessions/YYYY-MM/session-{FIRST_8_CHARS_OF_SESSION_ID}.md`
...
1. **Start broad:** `~/Documents/Claude/PRISM/logs/session-events.jsonl` — find the session by time range
...
4. **Deep dive:** `~/.claude/projects/-Users-[your-username]/{session_id}.jsonl` — full conversation transcript with all inputs and outputs
...
3. **Relative file paths.** Claude Code's working directory resets between Bash calls. Every path in a session log must be absolute (`~/Documents/Claude/PRISM/...`), never relative (`../sops/...`).
...
- **Canon source:** `~/Documents/Claude/PRISM/blitzmetrics-canon/05-maa-framework.md`
- **Triangles served:** MAA — Session logs are the PRISM's measurement system. Metrics = what was done, Analysis = what was learned, Action = SOP updates flagged. Without disciplined logging, the PRISM has no MAA loop and cannot improve.
...
- **2026-03-30:** v2.1 — Weekly retrospective found that 30 out of 31 sessions this week are flagged `needs_learnings: true`. The learnings capture system is still failing at massive scale despite the v2.0 improvements. Root cause: the Stop hook injects a systemMessage prompting Claude to write learnings, but Claude is either (a) not receiving it in time, (b) ignoring it under task pressure, or (c) the session ends before learnings can be written. **Recommendation:** The Stop hook needs to block session completion until learnings are confirmed written — or the retrospective task should batch-extract learnings from conversation logs when session-level capture fails. The EOD report already lists "Logs Missing Learnings" — this list should feed directly into the next session's context as a debt counter.
